...
export default async function Admin({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  return <AdminShell>
    <div className="pageHead"><div><div className="eyebrow">OPERATIONS COMMAND CENTER</div><h1>Growth Intelligence Pipeline</h1><p>Research → approve → onboard → activate → execute.</p></div><Link className="btn gold" href="/admin/clients/new">+ Create Client</Link></div>
    ...
    <div className="sectionTitle"><div><h2>Client Intelligence</h2><p>Every business keeps a permanent research, onboarding and execution record.</p></div><form method="get" className="formRow"><input name="q" defaultValue={q} placeholder="Search business name…" style={{width:260}}/><button className="btn secondary">Search</button></form></div>
    <div className="tableWrap"><table className="dataTable"><thead><tr><th>Business</th><th>Industry</th><th>Analysis</th><th>Portal</th><th>Onboarding</th><th>Program</th><th>Start</th></tr></thead><tbody>
      {clients.map((c:any)=><tr key={c.id}><td><Link href={`/admin/clients/${c.id}`}><b>{c.business_name}</b></Link></td><td>{c.industry||'general'}</td><td><span className={`status ${c.analysis_status==='complete'?'good':c.analysis_status==='running'?'live':'warn'}`}><i className="dot"/>{c.analysis_status||'not started'}</span></td><td><span className={`status ${c.portal_published?'good':'warn'}`}><i className="dot"/>{c.portal_published?'published':'private'}</span></td><td>{c.onboarding_status||'not started'}</td><td>${Number(c.management_fee||0).toLocaleString()}/mo</td><td>{c.start_date||'—'}</td></tr>)}
      {!clients.length&&<tr><td colSpan={7} className="muted">No clients yet. Create the first client to start the intelligence workflow.</td></tr>}
    </tbody></table></div>
  </AdminShell>
}
